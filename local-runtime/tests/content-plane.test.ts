...
// Exercises the composed content plane — adoptFolder
...
function makeFolder(files: Record<string, string>): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'kanwas-runtime-content-plane-'))
  tmpDirs.push(dir)
...
/** Materialize sidecars from an adopted proxy exactly as the runtime does at boot (mirrors folder-flusher.test.ts). */
...
describe('Mission A5 — content-plane end-to-end regression suite', () => {
  it('idle soak: ingest + repeated fragment churn + multiple flush cycles write zero bytes to disk', async () => {
    ...
    for (let i = 0; i < 5; i++) {
      // "runtime ingest" churn: the watcher re-detecting an unchanged file (a
      // suppression miss, an editor `touch`, a re-scan) replays the exact same
      // syncChange the runtime ran at boot — same content, no user edit. Assert the
      ...
      // "renderer mount-normalization" churn: BlockNote re-parses/re-serializes a
  ...
  it('round-trip growth: five user-edit save cycles through the real save path never grow trailing-`\\` count and converge', async () => {
    ...
    for (let i = 0; i < 5; i++) {
      // (a) Serialize the fragment exactly the way the renderer autosave hook does
      ...
      //     happen if suppression ever missed the runtime's own atomic write.
  ...
  it('external-edit-wins: a stale-baseHash save is rejected and disk keeps the external agent bytes untouched', async () => {
    ...
    // Establish a hash via a real user-edit save (the runtime's normal write path).
