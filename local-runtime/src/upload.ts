...
// `POST /workspaces/:id/files`. The runtime writes the uploaded bytes straight
// into the target canvas directory on disk; the renderer then puts the returned
...
export interface UploadResult {
  /** Workspace-relative path the renderer puts on the node (served by /files/raw). */
  storagePath: string
  mimeType: string
  size: number
