...
// runtime's own watcher ignores the echo; genuine external changes still flow.
//
// Design notes:
...
export class FolderFlusher {
   ...
   * by content, deletes by path) so the runtime does not re-ingest its own move.
    ...
    const walk = (canvas: CanvasItem, rel: string): void => {
      ...
      for (const node of fileNodes) {
        ...
        if (isBinary) {
          ...
          // left for the renderer's own write — nothing to reconcile.
