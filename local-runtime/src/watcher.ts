...
// under chokidar 4, so `.git/`, `node_modules/`, and the runtime's own `.kanwas/`
...
/** True when any path segment names a directory the runtime must never watch. */
...
export class FileWatcher {
  ...
  start(): void {
    ...
    this.watcher.on('ready', () => {
      void this.seedKnownPaths(watchTargets)
        .then(() => {
          this.log?.info({ watchTargets }, 'File watcher ready')
          this.options.onReady?.()
        })
        .catch((error: unknown) => this.reportAsyncError(error, 'Could not seed watcher paths'))
    })
  ...
  private handleAdd(targetPath: string, isDirectory: boolean): void {
    void this.processAdd(targetPath, isDirectory).catch((error: unknown) =>
      this.reportAsyncError(error, 'Could not process added path', { path: targetPath })
    )
  }
  ...
  private handleUpdate(targetPath: string, isDirectory: boolean): void {
    void this.processUpdate(targetPath, isDirectory).catch((error: unknown) =>
      this.reportAsyncError(error, 'Could not process updated path', { path: targetPath })
    )
  }
  ...
  private reportAsyncError(error: unknown, message: string, details: Record<string, unknown> = {}): void {
    const normalized = error instanceof Error ? error : new Error(String(error))
    this.log?.error({ ...details, error: normalized.message }, message)
