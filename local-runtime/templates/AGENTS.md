...
This folder is a **Kanwas vault**. The Kanwas desktop app watches it through its
embedded local runtime and renders it live as a spatial canvas. **The folder is
the source of truth** — everything is plain files. You are a CLI agent (Claude
Code, Codex, …) editing those files directly. There is no Kanwas-specific API,
plugin, or tool you need: just read and write files normally and the canvas
follows.
...
a different directory while Kanwas is open currently mints a new node** — the
old node's position and edges are lost, and the file starts fresh on the
destination canvas. If you need to reorganize across directories, prefer
creating the file in its target directory, or expect the moved node to reset.
(Reopening the vault re-adopts a moved file, but its id may still change.)
