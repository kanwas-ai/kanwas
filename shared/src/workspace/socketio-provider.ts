...
export class WorkspaceSocketProvider extends SocketSyncProviderBase {
  ...
  /**
   * True when `origin` is this provider's own remote-apply origin for the note's Y.Doc
   * (i.e. the update came from `applyBootstrap`/`applyDocMessage` — the server, not a
   ...
   */
  isRemoteNoteOrigin(noteId: string, origin: unknown): boolean {
