...
import { collapseHardBreakRunsInBlocks, escapeInlineHtmlForImport } from './markdown-fidelity.js'
...
export async function markdownToInterlinkedBlocks(
  ...
  const importReadyMarkdown = escapeInlineHtmlForImport(normalizeMarkdownForBlockNoteImport(markdown))
  const parsedBlocks = await editor.tryParseMarkdownToBlocks(importReadyMarkdown)
  const collapsedBlocks = collapseHardBreakRunsInBlocks(parsedBlocks as unknown[]) as ParsedBlocks
  return convertWorkspaceLinksToInterlinksInBlocks(collapsedBlocks) as ParsedBlocks
}
