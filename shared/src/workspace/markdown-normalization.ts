...
export function normalizeBlockNoteMarkdown(markdown: string): string {
  // '_' is included alongside '*'/'-' so `___`-style thematic breaks still hit the remark pass
  // now that `rule: '-'` normalizes their output character.
  if (!markdown.includes('*') && !markdown.includes('-') && !markdown.includes('_') && !/[0-9]+\./.test(markdown)) {
    return markdown
  }
