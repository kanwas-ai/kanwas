...
export interface FilesystemSyncerOptions {
  ...
  /**
   * Check whether a folder-relative directory still exists on disk. When
   * provided, `ensureParentCanvas` and `syncCanvas`'s create arm refuse to mint
   * a canvas for a directory that isn't actually there — guards against a stale
   * watcher event (queued behind a rename/move that already completed elsewhere)
   * minting a ghost canvas and resurrecting the old directory via its sidecar
   * write. Omit in contexts with no real filesystem (e.g. pure in-memory tests).
   */
...
export class FilesystemSyncer {
  ...
  private directoryExists?: (relPath: string) => boolean
  ...
  constructor(options: FilesystemSyncerOptions) {
    ...
    this.directoryExists = options.directoryExists
  }
  ...
  private async syncCanvas(change: FileChange): Promise<SyncResult> {
    ...
    switch (change.type) {
      case 'create':
        ...
        if (this.pathMapper.getCanvasMapping(change.path)) {
          return { success: true, action: 'no_op', canvasId: this.pathMapper.getCanvasMapping(change.path)!.canvasId }
        }
        if (this.directoryExists && !this.directoryExists(change.path)) {
          this.log.warn(
            { path: change.path },
            'Refusing to create a canvas for a directory that no longer exists on disk'
          )
          return { success: true, action: 'no_op' }
        }
        if (canvasInfo) {
  ...
  private async ensureParentCanvas(filePath: string): Promise<SyncResult> {
    ...
    for (const dirPath of toCreate) {
      if (this.directoryExists && !this.directoryExists(dirPath)) {
        // A stale event for a path under a directory that has since been
        // renamed/removed elsewhere — minting a canvas here would resurrect it.
        this.log.warn({ dirPath }, 'Refusing to create a canvas for a directory that no longer exists on disk')
        return { success: true, action: 'no_op' }
      }
  ...
  private async renameNodeFile(oldPath: string, newPath: string): Promise<SyncResult> {
    ...
    if (isBinaryNodeType(nodeItem.xynode.type)) {
      const newFilename = newPath.split('/').pop() || nextName
      if (nodeItem.xynode.type === 'image') {
        const existingData = nodeItem.xynode.data as ImageNodeData
        nodeItem.xynode.data = {
          ...existingData,
          storagePath: newPath,
        }
      } else if (nodeItem.xynode.type === 'audio') {
        const existingData = nodeItem.xynode.data as AudioNodeData
        nodeItem.xynode.data = {
          ...existingData,
          storagePath: newPath,
        ...
        const existingData = nodeItem.xynode.data as FileNodeData
        nodeItem.xynode.data = {
          ...existingData,
          storagePath: newPath,
          originalFilename: newFilename,
