...
describe('FilesystemSyncer - Binary Files', () => {
  ...
  // ==========================================================================
  // renameNodeFile() - binary nodes
  // ==========================================================================

  describe('Binary file renames', () => {
    it('should update storagePath when an image is renamed', async () => {
      const setup = createSyncerWithBinarySupport()
      disposeCallbacks.push(setup.dispose)

      const canvas = await createTestCanvas(setup, 'Test-Canvas')

      setup.fileMap.set('Test-Canvas/old-name.png', createFakeImageBuffer())
      const createResult = await setup.syncer.syncChange({
        type: 'create',
        path: 'Test-Canvas/old-name.png',
      })
      expect(createResult.success).toBe(true)

      const renameResult = await setup.syncer.syncRename('Test-Canvas/old-name.png', 'Test-Canvas/new-name.png', false)
      ...
      const data = node!.xynode.data as ImageNodeData
      expect(data.storagePath).toBe('Test-Canvas/new-name.png')
    })

    it('should update storagePath and originalFilename when a file node is renamed', async () => {
      const setup = createSyncerWithBinarySupport()
      disposeCallbacks.push(setup.dispose)

      const canvas = await createTestCanvas(setup, 'Test-Canvas')

      setup.fileMap.set('Test-Canvas/old-doc.pdf', createFakePDFBuffer())
      const createResult = await setup.syncer.syncChange({
        type: 'create',
        path: 'Test-Canvas/old-doc.pdf',
      })
      expect(createResult.success).toBe(true)

      const node = canvas.items.find((i): i is NodeItem => i.kind === 'node' && i.id === createResult.nodeId)
      expect(node).toBeDefined()
      const originalContentHash = (node!.xynode.data as FileNodeData).contentHash

      const renameResult = await setup.syncer.syncRename('Test-Canvas/old-doc.pdf', 'Test-Canvas/new-doc.pdf', false)
      expect(renameResult.success).toBe(true)

      const data = node!.xynode.data as FileNodeData
      expect(data.storagePath).toBe('Test-Canvas/new-doc.pdf')
      expect(data.originalFilename).toBe('new-doc.pdf')
      expect(data.contentHash).toBe(originalContentHash)
    })

    it('should update storagePath and originalFilename when an audio node is renamed', async () => {
      const setup = createSyncerWithBinarySupport()
      disposeCallbacks.push(setup.dispose)

      const canvas = await createTestCanvas(setup, 'Test-Canvas')

      setup.fileMap.set('Test-Canvas/old-song.mp3', createFakeMP3Buffer())
      const createResult = await setup.syncer.syncChange({
        type: 'create',
        path: 'Test-Canvas/old-song.mp3',
      })
      expect(createResult.success).toBe(true)

      const renameResult = await setup.syncer.syncRename('Test-Canvas/old-song.mp3', 'Test-Canvas/new-song.mp3', false)
      expect(renameResult.success).toBe(true)
      ...
      const data = node!.xynode.data as AudioNodeData
      expect(data.storagePath).toBe('Test-Canvas/new-song.mp3')
      expect(data.originalFilename).toBe('new-song.mp3')
    })
  })
})
