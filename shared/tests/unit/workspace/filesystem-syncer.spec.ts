...
function createTestSyncer(options?: {
  auditActor?: string
  now?: () => string
  ...
  const syncer = new FilesystemSyncer({
    ...
    ...(options?.autoCreateCanvases !== undefined ? { autoCreateCanvases: options.autoCreateCanvases } : {}),
...
describe('FilesystemSyncer', () => {
  ...
  describe('syncChange routing', () => {
    ...
    it('does not mint a duplicate canvas when the directory already maps to one (mkdir echo)', async () => {
      const setup = createTestSyncer()
      disposeCallbacks.push(setup.dispose)

      const rootCanvasCount = () => setup.proxy.root.items.filter((i) => i.kind === 'canvas').length

      // First create mints the canvas and registers path → canvas mapping.
      ...
      expect(echo.canvasId).toBe(canvas.id)
      expect(rootCanvasCount()).toBe(1)
    })
  ...
  // ==========================================================================
  // syncCanvas() - CREATE with `directoryExists` guard (Mission A4.2)
  //
  // A watcher event whose directory no longer exists on disk (a stale event
  // queued behind a rename/move that already completed elsewhere) must not
  // mint a canvas — that's the `git mv` ghost-sidecar defect.
  // ==========================================================================

  describe('syncCanvas - create with directoryExists guard', () => {
    it('refuses to create a canvas when directoryExists says the directory is gone', async () => {
      const setup = createTestSyncer({ directoryExists: () => false })
      disposeCallbacks.push(setup.dispose)

      const result = await setup.syncer.syncChange({ type: 'create', path: 'Ghost' })

      expect(result).toEqual({ success: true, action: 'no_op' })
      expect(setup.proxy.root.items.filter((i) => i.kind === 'canvas')).toHaveLength(0)
    })

    it('still creates the canvas when directoryExists confirms it is present', async () => {
      const setup = createTestSyncer({ directoryExists: () => true })
      disposeCallbacks.push(setup.dispose)

      const result = await setup.syncer.syncChange({ type: 'create', path: 'Real' })
    ...
    it('is a no-op regardless of directoryExists when the path already maps to a canvas (idempotent create)', async () => {
      ...
      disposeCallbacks.push(setup.dispose)
      const canvas = await createTestCanvas(setup, 'Existing')

      exists = false
      const result = await setup.syncer.syncChange({ type: 'create', path: 'Existing' })

      expect(result).toEqual({ success: true, action: 'no_op', canvasId: canvas.id })
    })
  })

  // ==========================================================================
  // ensureParentCanvas() via createNode with `directoryExists` guard (A4.2)
  // ==========================================================================

  describe('ensureParentCanvas with directoryExists guard', () => {
    it('refuses to auto-create a parent canvas whose directory is gone on disk', async () => {
      const setup = createTestSyncer({ autoCreateCanvases: true, directoryExists: () => false })
      disposeCallbacks.push(setup.dispose)
    ...
    it('auto-creates the parent canvas when directoryExists confirms it is present', async () => {
      const setup = createTestSyncer({ autoCreateCanvases: true, directoryExists: () => true })
      disposeCallbacks.push(setup.dispose)
      ...
      expect(canvases[0]!.name).toBe('real')
    })

    it('behaves exactly as before (no guard) when directoryExists is not provided', async () => {
      const setup = createTestSyncer({ autoCreateCanvases: true })
      disposeCallbacks.push(setup.dispose)

      const result = await setup.syncer.syncChange({
        type: 'create',
        path: 'AnyPath/note.md',
        content: '# Note\n',
      })
