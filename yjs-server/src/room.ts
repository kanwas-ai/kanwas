...
export class WorkspaceRoom {
   ...
   * local-daemon folder being unmounted), where leaving sockets attached with
   ...
   * closing the room. Public so `RoomManager.flushWorkspace()` can force a save
   * on demand (local-daemon's `POST /workspaces/:id/flush`); also used
