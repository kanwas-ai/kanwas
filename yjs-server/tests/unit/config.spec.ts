...
describe('loadConfig', () => {
  ...
  it('loads filesystem storage defaults and a noop backend notifier', () => {
    ...
    delete process.env.YJS_SERVER_SAVE_MAX_WAIT_MS
    ...
    expect(config.saveMaxWaitMs).toBe(5000)
  ...
  it('loads trimmed R2 settings and an HTTP backend notifier', () => {
    ...
    process.env.YJS_SERVER_SAVE_MAX_WAIT_MS = '9000'
    ...
    expect(config.saveMaxWaitMs).toBe(9000)
  ...
  it('falls back to the default save max-wait when invalid', () => {
    process.env.BACKEND_API_SECRET = 'secret'
    process.env.YJS_SERVER_STORAGE_DRIVER = 'fs'
    process.env.YJS_SERVER_SAVE_MAX_WAIT_MS = '-1'
