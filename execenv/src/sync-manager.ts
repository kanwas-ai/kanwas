...
// Section layout mutation/query types. These were previously exported by the
// (now-deleted) agent loopback `live-state-server`; retained here for the
// SyncManager section methods kept for Step 2 daemon work.
...
export class SyncManager {
  ...
  private async handleFileChangeInternal(
    ...
    const markdownPreflight = await this.preflightMarkdownChange(type, relativePath, absolutePath, content, changeLog)
