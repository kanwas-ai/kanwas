...
The workspace's yDoc syncs via the Yjs server (`useWorkspace().yDoc`).
...
**BlockNote patch:** `@blocknote/core@0.46.0` is patched (`patches/@blocknote__core@0.46.0.patch`) to accept an `undoManager` option in collaboration settings. This allows passing the shared `editorUndoManager` to all editors.
