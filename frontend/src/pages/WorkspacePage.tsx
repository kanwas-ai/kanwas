...
import { useUI, exitFocusMode, toggleTerminalOpen, toggleCardStyleEditor } from '@/store/useUIStore'
...
import { useUiContextReporter } from '@/components/terminal/uiContextReporter'
...
import CardStyleEditor from '@/components/dev/CardStyleEditor'
import { ENABLE_KEY as CARD_STYLE_EDITOR_ENABLE_KEY } from '@/components/dev/card-style-editor'
...
const CAN_SHOW_CARD_STYLE_EDITOR = import.meta.env.DEV || localStorage.getItem(CARD_STYLE_EDITOR_ENABLE_KEY) === '1'
...
function WorkspaceContent({ routeCanvasPath }: { routeCanvasPath: string }) {
  ...
  // Report selection/canvas/document context to kanwasd for the embedded terminal
  // (WP-C: "Insert context" push + MCP pull) — no-ops against the cloud backend.
  ...
  const { zenMode, fullScreenMode, sidebarOpen, sidebarWidth, toggleSidebar, disableFullScreenMode, terminalOpen } =
    useUI()
  ...
  useKeyboardShortcut('`', toggleTerminalOpen, { ctrl: true, skipInputs: false, preventDefault: true })

  // Toggle the (dev-only) card style editor panel
  useKeyboardShortcut('E', toggleCardStyleEditor, { ctrl: true, shift: true, skipInputs: false, preventDefault: true })
  ...
  const handleNodeSelect = useCallback(
    ...
    [navigateToCanvas, setSelectedNodeIdsIfChanged, workspaceId]
  )
  ...
  const handleSelectionChange = useCallback(
    ...
    [setSelectedNodeIdsIfChanged, workspaceId, textSelectionStore]
  )
...
export function WorkspacePage({
  ...
  return (
    <WorkspaceProvider workspaceId={finalWorkspaceId}>
      <WorkspaceContent routeCanvasPath={routeCanvasPath} />
    </WorkspaceProvider>
  )
}
