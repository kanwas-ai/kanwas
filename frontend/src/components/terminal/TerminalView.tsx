...
/**
 * Mounts one xterm.js Terminal per session and attaches it to the daemon over a WebSocket.
 ...
 * left running server-side; remounting (e.g. reopening the panel) reattaches and the daemon
...
export const TerminalView = memo(
  forwardRef<TerminalViewHandle, TerminalViewProps>(function TerminalView(
    ...
    useImperativeHandle(
      ...
      () => ({
        ...
        sendText: (text: string) => {
          ...
          // the daemon routes text frames to the JSON control parser instead.
    ...
    useEffect(() => {
      ...
      // Stdin MUST be a binary frame — the daemon routes text frames to the JSON
