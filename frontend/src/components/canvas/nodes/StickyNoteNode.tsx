...
import { STICKY_NOTE_NODE_LAYOUT } from 'shared/constants'
...
// Free-aspect resize bounds for sticky notes (see plan/sticky-text-settings.md Workstream A).
const STICKY_RESIZE_MIN = 160
const STICKY_RESIZE_MAX = 960
...
export default memo(function StickyNoteNode({ id, data, selected, width, height }: StickyNoteNodeProps) {
  ...
  const nodeWidth = width ?? STICKY_NOTE_NODE_LAYOUT.WIDTH
  const nodeHeight = height ?? STICKY_NOTE_NODE_LAYOUT.HEIGHT

  return (
    <div className="group relative" style={{ width: nodeWidth }}>
      ...
      <div className="relative">
        {/* Resize handle — bottom-right corner, visible on hover. Free aspect ratio. */}
        <NodeResizeControl
          ...
          minWidth={STICKY_RESIZE_MIN}
          minHeight={STICKY_RESIZE_MIN}
          maxWidth={STICKY_RESIZE_MAX}
          maxHeight={STICKY_RESIZE_MAX}
          shouldResize={() => true}
          autoScale={false}
        >
          <ResizeHandle color={colors.text} />
        </NodeResizeControl>
          ...
          style={
            {
              ...
              '--sticky-font-size': `${computeMaxStickyFont(nodeWidth)}px`,
              'lineHeight': 1.15,
            } as React.CSSProperties
          }
        >
          <StickyNoteEditor nodeId={id} fontFamily={fontFamily} nodeWidth={nodeWidth} nodeHeight={nodeHeight} />
        </div>
      </div>
