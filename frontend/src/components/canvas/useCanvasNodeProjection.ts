...
import type { FolderPeek, FolderPeekKind } from './types'
...
interface UseCanvasNodeProjectionOptions {
  ...
  /** Whether the currently active canvas is the workspace root (drives folder card semantic icons) */
...
function getFolderPeekChildKind(child: NodeItem | CanvasItem): FolderPeekKind {
  if (child.kind === 'canvas') {
    return 'canvas'
  }

  switch (child.xynode.type) {
    case 'blockNote':
      return 'document'
    case 'text':
    case 'stickyNote':
...
/** Peek at a folder's first 3 direct children + total count, for the shelf. */
function computeFolderPeek(item: CanvasItem): FolderPeek {
  return {
    kinds: item.items.slice(0, 3).map(getFolderPeekChildKind),
    ...
    const result: ProjectedCanvasNode[] = canvas.items.map((item) => {
      ...
      const node: ProjectedCanvasNode = {
        ...
        data: {
          ...
          inGroup: isInGroup,
          ...(item.kind === 'canvas' && {
            folderPeek: computeFolderPeek(item),
            isTopLevelCanvas,
          }),
          ...(isCollapsed && {
