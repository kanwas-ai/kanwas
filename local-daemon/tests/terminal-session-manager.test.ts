 ...
 * booting the full daemon (mounts/yjs/etc — rest-server.ts's job is just thin
...
describe('TerminalSessionManager', () => {
  ...
  beforeEach(async () => {
    ...
    workspaceFolder = fs.mkdtempSync(path.join(os.tmpdir(), 'kanwasd-terminal-'))
  ...
  it('list() scopes by workspaceId; belongsTo() checks ownership', () => {
    const other = fs.mkdtempSync(path.join(os.tmpdir(), 'kanwasd-terminal-other-'))
    try {
