...
//
// When kanwasd is pointed at an EMPTY folder (a fresh workspace), drop the
...
// maps to the canvas, and can auto-register kanwasd's MCP endpoint. This
...
//
// Gated on emptiness so pointing the daemon at an EXISTING folder never injects
...
/** Packaged template paths — resolve the same from `src/` (tsx) and `dist/` (built). */
const templatePath = fileURLToPath(new URL('../templates/AGENTS.md', import.meta.url))
// Packaged as `mcp.json.template` (not `.mcp.json`) — this repo's own .gitignore
// blanket-ignores `.mcp.json` as a personal Claude Code/MCP config, which would
// silently drop the packaged template. The seeded DESTINATION filename is still
// `.mcp.json` (MCP_CONFIG_FILE below) — only the source's name in this repo differs.
const mcpConfigTemplatePath = fileURLToPath(new URL('../templates/mcp.json.template', import.meta.url))
 ...
 */
export async function seedWorkspaceTemplateIfEmpty(folder: string, logger: Logger): Promise<boolean> {
  const log = logger.child({ component: 'SeedTemplate' })
  if (!(await folderIsEmpty(folder))) return false

  const wroteAgents = await seedFileIfMissing(folder, AGENTS_FILE, templatePath, log)
  await seedFileIfMissing(folder, MCP_CONFIG_FILE, mcpConfigTemplatePath, log)
  return wroteAgents
}
