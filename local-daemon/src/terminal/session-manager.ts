...
export interface CreateSessionOptions {
  ...
  rows?: number
  /**
   * Optional UI theme hint, forwarded from the frontend's create-session
...
export class TerminalSessionManager {
  ...
  createSession(options: CreateSessionOptions): TerminalSession {
    ...
    const cols = options.cols && options.cols > 0 ? Math.floor(options.cols) : 80
    const rows = options.rows && options.rows > 0 ? Math.floor(options.rows) : 24
  ...
  resize(sessionId: string, cols: number, rows: number): void {
    ...
    record.proc.resize(c, r)
    record.session.cols = c
    record.session.rows = r
  }
  ...
  /** Daemon shutdown: terminate every live process and close every attached socket. */
  disposeAll(): void {
    for (const id of [...this.sessions.keys()]) {
      ...
      if (record.session.status === 'running') this.kill(id)
    }
    this.logger.info({ count: this.sessions.size }, 'Disposing all terminal sessions')
  }
