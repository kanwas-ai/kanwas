...
// terminal, or anywhere else pointed at this daemon) PULL the same UI context
// the frontend PUSHES into the terminal's stdin — see ui-context.ts for the
// shared store/resolution logic.
//
// Stateless mode (`sessionIdGenerator: undefined`, no session bookkeeping):
// this daemon has no need for the resumable-stream story stateful mode
...
export function createKanwasMcpServer(options: McpHandlerOptions): McpServer {
  ...
  server.registerTool(
    'kanwas_workspace_info',
    {
      ...
      description: 'List the Kanwas workspace(s) this daemon has mounted: id, absolute folder path, and display name.',
    },
    async () => {
