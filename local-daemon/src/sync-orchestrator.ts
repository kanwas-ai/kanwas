...
//   - NO Kanwas.md restore, audit identity, metadata retry queue
// Disk changes simply apply to the yDoc (last-writer-wins; the flusher is a later
// mission). The only writes the daemon makes are suppression-tagged metadata.yaml
// sidecars (and `.kanwas/`).
...
export interface SyncOrchestratorOptions {
  ...
  secret: string
  logger: Logger
  /** Shared echo-cancellation for the daemon's own writes (folder→yDoc + flusher). */
...
export class SyncOrchestrator {
  ...
  async start(): Promise<void> {
    // 1. Connect as a non-frontend client (keeps the room alive across frontend reloads).
  ...
  private async handleChange(type: 'create' | 'update' | 'delete', absPath: string): Promise<void> {
    ...
    if (type === 'delete') {
      // Drop the daemon's own removals (rename/move/trash) — real deletes proceed.
    ...
    if (prepared.binary) {
      // Drop the daemon's own binary echoes (uploads, binary moves) by exact bytes.
    ...
    // Text (.md/.yaml): drop the daemon's own echoes by exact-byte match.
    if (
   ...
   * its own create echo — the frontend creates the node itself, so exactly one
  ...
  private async doNoteSave({ nodeId, body, baseHash, force }: NoteSaveRequest): Promise<NoteSaveResult> {
    ...
    if (diskBytes !== undefined && baseHash !== undefined && !force) {
      const diskHash = hashRaw(diskBytes)
  ...
  async stop(): Promise<void> {
    ...
    await Promise.all([this.immediateWatcher?.stop(), this.settledWatcher?.stop()])
