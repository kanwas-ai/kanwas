...
// are node ids, both only unique WITHIN a folder), so a daemon hosting several
...
export interface MountOptions {
  ...
  /** The daemon-wide FolderStore; the mount binds its flusher into it, keyed by workspaceId. */
  store: FolderStore
  /** Per-run HMAC secret shared with the REST minter and the socket verifier. */
  secret: string
  /** Host the sync-orchestrator's watcher client dials for the yjs socket. */
  clientHost: string
  /** Yjs socket.io port. */
  yjsPort: number
  logger: Logger
...
export interface Mount {
  folder: string
  workspaceId: string
  /** Hyphen-stripped workspace id the frontend routes to. */
...
export async function mountFolder(options: MountOptions): Promise<Mount> {
  const { folder, roomManager, store, secret, clientHost, yjsPort, logger } = options
  ...
  await seedWorkspaceTemplateIfEmpty(folder, logger)
  ...
  await roomManager.replaceDocument(identity.workspaceId, bundle, {
    stage: 'replace',
    reason: 'daemon-adopt',
    notifyBackend: false,
  })
