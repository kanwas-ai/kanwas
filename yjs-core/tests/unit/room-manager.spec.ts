...
describe('RoomManager', () => {
  ...
  it('flushWorkspace flushes a live room without closing it', async () => {
    const manager = new RoomManager({
      logger,
      saveDebounceMs: 5,
      saveMaxWaitMs: 100,
    ...
    const room = await manager.getRoom('workspace-1')
    const flushSpy = vi.spyOn(room, 'flushPendingSave')

    await manager.flushWorkspace('workspace-1')
  ...
  it('flushWorkspace no-ops for a workspace with no live room', async () => {
    const manager = new RoomManager({
      logger,
      saveDebounceMs: 5,
      saveMaxWaitMs: 100,
    ...
    await expect(manager.flushWorkspace('never-mounted')).resolves.toBeUndefined()
    expect(manager.activeRoomCount).toBe(0)
  })
