...
export class WorkspaceRoom {
  ...
  /** Timestamp of the first schedule since the last completed save (maxWait clock). */
  private pendingSaveSince: number | null = null
  private blockedForReplacement = false
  private destroyed = false
  /** In-flight `flushAndDestroy` teardown, if any — see the reentrancy note there. */
  ...
  async initialize(options?: InitializeRoomOptions, context?: OperationContext): Promise<void> {
    ...
    this.initializeTask = (async () => {
      ...
      const rootBytes = await this.store.loadRoot(this.workspaceId)
  ...
  async replaceDocument(
    ...
    try {
      ...
      await this.enqueuePersistenceTask(async () => {
        await this.persistSnapshotBundle(hydratedSnapshot, context)
      })
      ...
      log.info({ connectionCount: this.connectionCount, reason: options.reason }, 'Workspace snapshot replaced')
    } finally {
  ...
  /**
   * Force-close this room regardless of live connections: broadcast a reload
   * signal, disconnect every attached socket (a server-initiated
   * `socket.disconnect()` — socket.io clients do NOT auto-reconnect after this,
   * only after an unexpected transport-level drop, so this cannot resurrect the
   * room via a client reconnect), then flush any pending state and tear down.
   *
   * Unlike `replaceDocument`, this does not apply a new snapshot — it is the
   * room-manager's response to the room's BACKING STORE going away (e.g. a
   * local-runtime folder being unmounted), where leaving sockets attached with
   * no persistence path downstream would let clients silently edit into a void.
   */
  async closeImmediately(): Promise<void> {
    ...
    this.broadcastReload('room_closed')
    this.disconnectAllSockets()
    await this.flushAndDestroy()
  }
  ...
  async flushAndDestroy(): Promise<void> {
    ...
    // Reentrant-safe: `closeImmediately()`'s `disconnectAllSockets()` disconnects
    // sockets SYNCHRONOUSLY, which can re-enter here (via each socket's own
    // 'disconnect' handler → roomManager.destroyRoomIfEmpty → this same method)
    // before this call has reached the `this.destroyed = true` line below (that
    // only happens after the awaited persistence work). Without this latch, a
    // reentrant call would see `destroyed` still false and redo the entire
    // teardown (double-flush, double doc-destroy) concurrently with this one.
    if (this.closeTask) {
      return this.closeTask
    }

    this.closeTask = this.performFlushAndDestroy()
    return this.closeTask
  }

  private async performFlushAndDestroy(): Promise<void> {
    const connectionCount = this.connectionCount
    const docSize = this.currentRootSizeBytes
    const noteCount = this.noteStates.size
    ...
    try {
      this.log.info({ connectionCount, docSize, noteCount }, 'Destroyed workspace room')
    } catch (error) {
      appendError(errors, error)
    }

    throwAggregateErrors(errors, `Failed to destroy workspace room ${this.workspaceId} cleanly`)
  }
  ...
  private teardownWorkspaceState(): void {
    ...
    throwAggregateErrors(errors, `Failed to tear down workspace room ${this.workspaceId} documents`)
  }
  ...
  private async loadNoteState(noteId: string, noteState: DocState, context?: OperationContext): Promise<DocState> {
    const noteBytes = await this.store.loadNote(this.workspaceId, noteId)
    if (!noteBytes) {
  ...
  private scheduleSave(): void {
    ...
    // Trailing debounce, capped so a save is guaranteed at least every
    // saveMaxWaitMs even while updates keep arriving faster than saveDebounceMs
    // (otherwise disk can lag the live doc indefinitely during continuous edits).
    const delay = Math.min(this.saveDebounceMs, Math.max(0, this.pendingSaveSince + this.saveMaxWaitMs - now))
  ...
  /**
   * Run the currently-pending debounced save (if any) immediately, without
   * closing the room. Public so an embedding runtime can force a save on demand;
   * also used
   * internally by `performFlushAndDestroy`.
   */
    ...
    await this.enqueuePersistenceTask(async () => {
      ...
      try {
        for (const noteId of noteIdsToSave) {
          ...
          await this.store.saveNote(this.workspaceId, noteId, Y.encodeStateAsUpdateV2(noteState.doc))
        }
        ...
        if (rootShouldSave) {
          ...
          await this.store.saveRoot(this.workspaceId, rootBytes)
          rootSaved = true
        }

        for (const noteId of noteIdsToDelete) {
          await this.store.deleteNote(this.workspaceId, noteId)
          deletedNoteCount += 1
        }

        if (rootShouldSave || noteIdsToSave.length > 0 || noteIdsToDelete.length > 0) {
          this.logPersistence(context, 'save', {
  ...
  private async persistSnapshotBundle(snapshot: HydratedSnapshotBundle, context?: OperationContext): Promise<void> {
    const noteSaveResults = await Promise.allSettled(
      snapshot.noteIds.map(async (noteId) => {
        ...
        await this.store.saveNote(this.workspaceId, noteId, noteBytes)
      })
    )
    ...
    await this.store.saveRoot(this.workspaceId, snapshot.rootBytes)
    this.logPersistence(context, 'replace', {
      noteCount: snapshot.noteIds.length,
      rootSaved: true,
    })
  }

  private logPersistence(
