...
- **bash**: Read-only shell commands (ls, find, grep, cat, head, tail, wc, file, tree). State-modifying commands are blocked.
- **text_editor**: View files only (view command). Create/edit commands are blocked.
...
- One broad search, then browse structure with `ls` or `tree`
...
2. Directory listing (ls, tree) - understand structure around discovered files
...
2. **Batch operations**: Use `find` or `grep` to locate multiple files at once
3. **Read with context**: Read enough surrounding context to understand a file before moving on
4. **Trust your findings**: Don't second-guess and re-read files unnecessarily
5. **Iteration budget**: You have limited iterations - make each one count
