...
------------------------------      ----------------------------
Your bash tool runs HERE            COMPOSIO_REMOTE_BASH_TOOL runs HERE
Your text_editor runs HERE          COMPOSIO_REMOTE_WORKBENCH runs HERE
...
**bash tool** - Runs commands in the local sandbox at /workspace. Use this to:
...
Use LOCAL `/workspace` or `return_output` only for the final user-visible result:
...
- **bash** - Execute shell commands in /workspace (read files, write results)
- **str_replace_based_edit_tool** - View/create/edit files in /workspace (.md, .yaml only)
...
5. **Minimal iterations:** Complete the task with as few tool calls as possible
...
- You CANNOT modify workspace files with remote tools (use native bash/str_replace_based_edit_tool)
