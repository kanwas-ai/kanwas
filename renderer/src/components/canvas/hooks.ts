...
export const useUserFacingNodeEdit = () => {
  const { store, workspaceUndoController, localUser } = useContext(WorkspaceContext)!
  const auditActor = createUserAuditActor(localUser.id)
...
export const useAddNode = () => {
  const { store, yDoc, workspaceUndoController, localUser } = useContext(WorkspaceContext)!
  const auditActor = createUserAuditActor(localUser.id)
...
export const useAddImageNode = () => {
  const { store, workspaceId, localUser } = useContext(WorkspaceContext)!
...
export const useAddFileNode = () => {
  const { store, workspaceId, localUser } = useContext(WorkspaceContext)!
  const uploadImage = useUploadImage() // The local upload endpoint accepts all supported file types.
  const auditActor = createUserAuditActor(localUser.id)
...
export const useAddLinkNode = () => {
  const { store, localUser } = useContext(WorkspaceContext)!
  const auditActor = createUserAuditActor(localUser.id)
...
export const useFetchLinkMetadata = () => {
  ...
  return useMutation({
    mutationFn: async (params: { nodeId: string; url: string; workspaceId: string; canvasId: string }) => {
      const response = await localApi.linkMetadata(params.workspaceId, {
        url: params.url,
        canvasId: params.canvasId,
      })
      return { ...response, nodeId: params.nodeId }
    },
