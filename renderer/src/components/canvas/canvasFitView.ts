...
/**
 * Folder card ("Contents shelf") fixed dimensions — single source of truth,
 * consumed by CanvasNode.tsx for its own rendering. Filled height accounts for
 * the shelf row; empty height drops the shelf and tightens vertical padding
 * so the "Empty" line fits without overflow (see CanvasNode.tsx for the math).
 */
export const FOLDER_CARD_WIDTH = 288
export const FOLDER_CARD_HEIGHT = 118
export const FOLDER_CARD_HEIGHT_EMPTY = 68

const CANVAS_CARD_FALLBACK = {
  // Matches the rendered folder card dimensions in CanvasNode (filled state —
  // fit-view/layout reservations always use the larger of the two heights).
