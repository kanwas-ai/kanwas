...
export function CanvasFlow({
  ...
  // LLM node summarization was removed with the built-in agent (local-first
  // Step 1). Collapsed nodes simply get an empty summary now.
  const requestNodeSummary = useCallback((node: NodeItem) => {
  ...
  const nodes = useCanvasNodeProjection({
    ...
    selectedNodeIds,
    isTopLevelCanvas: canvas.id === store.root.id,
  })
