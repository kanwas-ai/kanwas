...
export default memo(function TextNode({ id, data, selected }: TextNodeProps) {
  ...
  const handleSizeChange = useCallback(
    (newFontSize: number) => {
      const nodeData = getNodeData()
      if (nodeData) nodeData.fontSize = newFontSize
    },
    [getNodeData]
  )
  ...
  const handleColorChange = useCallback(
    (newColor: string) => {
      const nodeData = getNodeData()
      if (nodeData) nodeData.color = newColor
    },
    [getNodeData]
  )
      ...
      {selected && !isEditing && (
        <>
          ...
          <NodeSideToolbar
            ...
            textSize={{ current: fontSize, onChange: handleSizeChange }}
            textColor={{ current: color, onChange: handleColorChange }}
          />
        </>
      )}
    </div>
